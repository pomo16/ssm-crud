...
<script
	src="${APP_PATH }/static/bootstrap-4.1.3-dist/js/bootstrap.min.js"></script>
<!-- Font Awesome CSS -->
<link rel="stylesheet"
	href="${APP_PATH }/static/fontawesome/css/all.min.css">
</head>
<body>
	<!-- 员工修改模态框 -->
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">员工修改</h5>
				...
				<div class="modal-body">
					<form>
						<div class="form-group row">
						    ...
						    <div class="col-sm-10">
						        <input type="text" readonly class="form-control-plaintext" id="empName_update_static" value="">
						    </div>
						</div>
						<div class="form-group row">
							<label for="email_add_input" class="col-sm-2 col-form-label">email</label>
							<div class="col-sm-10">
							...
							<div class="col-sm-10">
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="radio" name="gender"
										id="gender1_update_input" value="M" checked="checked"> <label
										class="form-check-label" for="inlineRadio1">男</label>
								</div>
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="radio" name="gender"
										id="gender2_update_input" value="F"> <label
						...
						<div class="form-group row">
							...
							<div class="col-sm-4">
								...
								<select class="custom-select my-1 mr-sm-2"
									id="dept_update_select" name="dId"></select>
							</div>
						</div>
					</form>
				</div>
					...
					<form>
						...
						<div class="form-group row">
							...
							<div class="col-sm-10">
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="radio" name="gender"
										id="gender1_add_input" value="M" checked="checked"> <label
										class="form-check-label" for="inlineRadio1">男</label>
								</div>
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="radio" name="gender"
										id="gender2_add_input" value="F"> <label
										class="form-check-label" for="inlineRadio2">女</label>
								</div>
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-sm-4">
								<!-- 部门提交部门id即可 -->
								<select class="custom-select my-1 mr-sm-2"
									id="dept_add_select" name="dId"></select>
							</div>
						</div>
					</form>
				</div>
	...
	<script type="text/javascript">
		...
		function build_emps_table(result) {
			...
			$.each(emps, function(index, item) {
				...
				var editBtn = $("<button></button>").addClass(
						...
						$("<span></span>").addClass("fas fa-pencil-alt"))
						.append("编辑");
		...
		function build_page_nav(result) {
			...
			var firstPageLi = $("<li></li>").addClass("page-item").append(
					$("<a></a>").addClass("page-link").append("首页").attr(
							"href", "#"));
			var prePageLi = $("<li></li>").addClass("page-item").append(
					$("<a></a>").addClass("page-link").append("&laquo;").attr(
							"href", "#"));
			...
			var nextPageLi = $("<li></li>").addClass("page-item").append(
					$("<a></a>").addClass("page-link").append("&raquo;").attr(
							"href", "#"));
			var lastPageLi = $("<li></li>").addClass("page-item").append(
					$("<a></a>").addClass("page-link").append("末页").attr(
							"href", "#"));
			...
			$.each(result.extend.pageInfo.navigatepageNums, function(index,
				...
				var numLi = $("<li></li>").addClass("page-item").append(
						$("<a></a>").addClass("page-link").append(item).attr(
								"href", "#"));
		...
		function getDepts(ele){
			...
			$.ajax({
				...
				success:function(result){
					//console.log(result);
					//显示部门信息在下拉列表中
					//$("#empAddModal select") or ${"#dept_add_select" }
			...
			$.ajax({
				...
				success:function(result){
					...
					if(result.code == 100){
						...
						//2.来到最后一页显示刚才保存的数据
						//发送ajax请求显示最后一页数据即可
						//给一个足够大的数字给to_page()即可，这里的思路是总记录数总大于总页数
						to_page(totalRecord);
					} else {
						//显示失败信息
						//console.log(result);
		...
		$("#emp_update_btn").click(function(){
			...
			$.ajax({
				...
				/*
				使用HiddenHttpMethodFilter在tomcat上进行POST封装后再通过_method=PUT解析为PUT请求
				type:"POST",
				data:$("#empUpdateModal form").serialize()+"&_method=PUT",
				*/
				//使用FormContentFilter直接发送PUT请求
				type:"PUT",
